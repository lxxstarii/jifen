...
@Service
public class JfParentPointsAssignServiceImpl implements
		IJfParentPointsAssignService {

	@Autowired
	private IJfParentPointsAssignRepository repository;

	@Override
	public ListResult<JfParentPointsAssign> query(
			JfParentPointsAssign jfParentPointsAssign, Integer pageIndex,
		...
		PageRequest pageRequest = new PageRequest(pageSize * (pageIndex - 1),
				pageSize);
		Page<JfParentPointsAssign> page = repository
				.findByParentIdAndStudentId(parentId, studentId, pageRequest);
		if (null == page) {
			throw new RepsException("mongodb查询失败");
		}
		ListResult<JfParentPointsAssign> listResult = new ListResult<>(
	...
	@Override
	public void save(JfParentPointsAssign data) {
		repository.save(data);
	}

	@Override
	public List<JfParentPointsAssign> find(JfParentPointsAssign query) {
		return null;
	}

	@Override
	public List<JfParentPointsAssign> findByTime(Date monday, Date weekend,
