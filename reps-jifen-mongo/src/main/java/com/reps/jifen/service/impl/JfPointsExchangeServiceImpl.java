...
public class JfPointsExchangeServiceImpl implements IJfPointsExchangeService {
	...
	protected final Logger logger = LoggerFactory
			.getLogger(JfPointsExchangeServiceImpl.class);

	@Autowired
	private IJfPointsExchangeRepository repository;

	@Override
	public ListResult<JfPointsExchange> findByPersonId(String personId,
		...
		PageRequest pageRequest = new PageRequest(pageSize * (pageIndex - 1),
				pageSize);
		Page<JfPointsExchange> page = repository.findByPersonId(personId,
				pageRequest);
		if (null == page) {
			throw new RepsException("mongodb查询失败");
		}
		ListResult<JfPointsExchange> listResult = new ListResult<>(
