...
@Service
public class JfTeacherPointsAssignServiceImpl implements
		IJfTeacherPointsAssignService {

	protected final Logger logger = LoggerFactory
			.getLogger(JfTeacherPointsAssignServiceImpl.class);

	@Autowired
	private IJfTeacherPointsAssignRepository repository;

	@Override
	public ListResult<JfTeacherPointsAssign> findByTeacherId(String teacherId,
		...
		PageRequest pageRequest = new PageRequest(pageSize * (pageIndex - 1),
				pageSize);
		Page<JfTeacherPointsAssign> page = repository.findByTeacherId(
				teacherId, pageRequest);
		if (null == page) {
			throw new RepsException("mongodb查询失败");
		}
		ListResult<JfTeacherPointsAssign> listResult = new ListResult<>(
