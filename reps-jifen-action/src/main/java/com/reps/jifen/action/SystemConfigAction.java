...
public class SystemConfigAction extends BaseAction {
    ...
    private static final Logger log = LoggerFactory.getLogger(SystemConfigAction.class);

    @Autowired
    ISystemConfigService systemConfigService;

    @RequestMapping(value="/list")
    public ModelAndView list(Pagination pager, SystemConfig config) {
        ModelAndView mav = getModelAndView("/jifen/appuse/list");
        ListResult<SystemConfig> result = systemConfigService.query(pager.getStartRow(), pager.getPageSize(), config);
    ...
    @ResponseBody
    public Object add(SystemConfig config) {
    ...
    @ResponseBody
    public Object update(SystemConfig config) {
