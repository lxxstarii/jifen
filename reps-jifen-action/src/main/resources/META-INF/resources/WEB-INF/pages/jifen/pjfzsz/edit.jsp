...
<html>
  ...
  <body>
    <reps:container >
  	<reps:panel id="myform" dock="none" border="true" action="update.mvc" formId="xform" validForm="true" method="post" title="修改评价指标" style="width:900px;">
		...
		<reps:formcontent style="margin-top:30px">
           <reps:formfield label="指标项 " labelStyle="width:20%;" textStyle="width:20%;">
				<reps:input id="item" name="item" maxLength="15" required="true">${data.item}</reps:input>
           </reps:formfield>
            <reps:formfield label="指标类型" labelStyle="width:20%;" textStyle="width:20%;">
           		<reps:select name="mark" jsonData="{1:'奖励',0:'惩罚'}" headerValue="" style="width:180px;" required="true">${data.mark}</reps:select>
		   </reps:formfield>
		   <reps:formfield label="奖励/扣除积分" labelStyle="width:20%;" textStyle="width:20%;">
           		<reps:select name="pointsScope" jsonData="{'1':'0~1','2':'0~2','3':'0~3','4':'0~4','5':'0~5'}"
           			headerValue="" style="width:180px;" required="true">
           			${data.pointsScope}
           		</reps:select>
		   </reps:formfield>
